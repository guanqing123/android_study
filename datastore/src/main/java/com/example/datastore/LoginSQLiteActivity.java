...
import com.example.datastore.database.LoginDBHelper;
...
public class LoginSQLiteActivity extends AppCompatActivity implements RadioGroup.OnCheckedChangeListener, View.OnClickListener, View.OnFocusChangeListener {
        ...
        switch (v.getId()) {
            case R.id.btn_forget: {
                ...
                if (rb_password.isChecked()) {
                    // 以下携带手机号码跳转到找回密码的界面
                    Intent intent = new Intent(LoginSQLiteActivity.this, SQLiteHelperActivity.class);
            ...
            case R.id.btn_login: {
                ...
                if (rb_password.isChecked()) {
                    ...
                    // 保存到数据库
                    LoginInfo info = new LoginInfo(phone,
                            et_password.getText().toString(),
                            ck_remember.isChecked());
                    helper.save(info);
                    // 提示用户登录成功
                    loginSuccess();
                } else if (rb_verify_code.isChecked()) {
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        if (R.id.et_password == v.getId() && hasFocus) {
    ...
    private class HideTextWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable s) {
            if (s.length() == maxLength) {
                // 隐藏输入法
                hideKeyboard(LoginSQLiteActivity.this, et);
            }
        }
    }
