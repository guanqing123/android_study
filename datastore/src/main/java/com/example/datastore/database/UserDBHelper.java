...
public class UserDBHelper extends SQLiteOpenHelper {
    ...
    public long insert(User user) {
        ...
        // 如果第三个参数 values 为 Null 或者元素个数为0，由于 insert() 方法要求必须添加一条除了主键之外其它字段为 Null 值的记录，
        // 为了满足 SQL 语法的需要，insert 语句必须给定一个字段名，如：insert into person(name) values (Null),
    ...
    public List<User> queryAll(){
        ...
        while (cursor.moveToNext()) {
            ...
            user.height = cursor.getLong(cursor.getColumnIndex("height"));
            user.weight = cursor.getFloat(cursor.getColumnIndex("weight"));
            user.married = cursor.getInt(cursor.getColumnIndex("married")) == 1;
            list.add(user);
        }
        return list;
    }
