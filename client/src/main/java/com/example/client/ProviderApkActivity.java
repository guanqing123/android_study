...
public class ProviderApkActivity extends AppCompatActivity implements View.OnClickListener {
        ...
        switch (v.getId()) {
            case R.id.btn_install: {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    Log.d("gq", "Android 11 之后获取 MANAGE_EXTERNAL_STORAGE 权限");
                    checkAndInstall();
                } else {
    ...
    private void installApk() {
        String apkPath = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() + "/datastore-debug.apk";
        Log.d("gq", "apkPath:" + apkPath);
        // 获取应用包管理器
